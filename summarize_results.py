...
mask1, mask2 = MASK
_, num_Event, num_Category = (
    mask1.shape
)  # Dimension of mask1: [subj, Num_Event, Num_Category]
...
for out_itr in range(OUT_ITERATION):
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(in_path, f"itr_{out_itr}/models/model_itr_{out_itr}.pth")
        )
    )
    ...
    ### EVALUATION ###
    result1, result2 = np.zeros([num_Event, len(EVAL_TIMES)]), np.zeros(
        [num_Event, len(EVAL_TIMES)]
    )
    ...
    for t, t_time in enumerate(EVAL_TIMES):
        ...
        if eval_horizon >= num_Category:
            ...
            risk = np.sum(
                pred[:, :, : (eval_horizon + 1)].cpu().numpy(), axis=2
            )  # Risk score until EVAL_TIMES
